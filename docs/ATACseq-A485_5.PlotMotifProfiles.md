### 5. Plotting motif density around ATAC peak summits

```console
cd $PROCESSED/A485/bam_files/FIMO/tomtom/fimo_composites/
...
cp PSWM_family_1_mm10_instances.bigWig main_figure_beds/Fra2_mm10_instances.bigWig
cp PSWM_family_2a_mm10_instances.bigWig main_figure_beds/Klf4_mm10_instances.bigWig
cp PSWM_family_2b_mm10_instances.bigWig main_figure_beds/Sp2_mm10_instances.bigWig
cp PSWM_family_3_mm10_instances.bigWig main_figure_beds/Maz_mm10_instances.bigWig
cp PSWM_family_4_mm10_instances.bigWig main_figure_beds/Trp63_mm10_instances.bigWig
cp PSWM_family_5_mm10_instances.bigWig main_figure_beds/RUNX_mm10_instances.bigWig
cp PSWM_family_6_mm10_instances.bigWig main_figure_beds/TEAD4_mm10_instances.bigWig
```
...
dir = '$PROCESSED/A485/bam_files/FIMO/tomtom/fimo_composites/'
setwd(dir)
...
dir = '$PROCESSED/A485/bam_files/FIMO/tomtom/fimo_composites/main_figure_beds/'
...
decreased <- decreased[!grepl('chrUn', decreased$chr),]
...
increased <- increased[!grepl('chrUn', increased$chr),]

not_different = read.table('$PROCESSED/A485/bam_files/nondynamic_peaks.bed')
not_different = not_different[not_different$V1 != 'chrM',]
not_different = bedWindow(not_different, half_win)
not_different <- not_different[!grepl('chrUn', not_different$V1),]

# Drop the unknown chromosome peaks
fimo_scores_atac <- fimo_scores_atac[!grepl('chrUn', fimo_scores_atac$peak),]
fimo_scores_atac$peak <- NULL
for(i in 1:ncol(fimo_scores_atac)) {
...
Plot all together
```R
plotFimoLattice(all_fimo,
                num.m = -500, num.p = 500,
                plot_name = 'motif_enrichment_around_summits_all-factors.pdf',
                col_lines = c("#FFC20A", "#0C7BDC", "#A2A2A0"))
```

Plot increased only
```R
plotFimoLattice(all_fimo[all_fimo$tf %in% c("TEAD4", "Klf4", "Sp2", "Maz", "Trp63"),],
                num.m = -500, num.p = 500,
                plot_name = 'motif_enrichment_around_summits_INCREASED.pdf',
                col_lines = c("#FFC20A", "#0C7BDC", "#A2A2A0"))
```

Plot decreased only
```R
plotFimoLattice(all_fimo[all_fimo$tf %in% c("Fra2", "RUNX"),],
                num.m = -500, num.p = 500,
                plot_name = 'motif_enrichment_around_summits_DECREASED.pdf',
                col_lines = c("#FFC20A", "#0C7BDC", "#A2A2A0"))
```

```R
final_atac_all_dt
table(final_atac_all_dt$response_DMSOvA485)
...
svglite(paste0("atac_peaks_by_factor_", Sys.Date(), ".svg"),
        bg = "transparent", fix_text_size = FALSE, pointsize = 6)
ggplot(plot_df, aes(x=as.factor(family), fill=as.factor(status) )) +
  geom_bar() +
  scale_fill_manual(name = "Status", labels = c("Increased", "Decreased"), values = c("#FFC20A", "#0C7BDC")) +
