...
trt_A485_counts   <- getRawCountsInterval(consensus_peaks_file, directory, file_prefix = 'A485')
ctrl_DMSO_counts <- getRawCountsInterval(consensus_peaks_file, directory, file_prefix = 'DMSO')

counts_df <- cbind(trt_A485_counts, ctrl_DMSO_counts)
save(counts_df, file='counts_df_10.10.2023.RData')

sample_conditions = c("A485", "A485", "DMSO", "DMSO")
deseq_counts_table = DESeqDataSetFromMatrix(counts_df,
    DataFrame(sample_conditions), ~ sample_conditions)
colData(deseq_counts_table)$condition <- c("A485", "A485", "DMSO", "DMSO")
...
#padj.cutoff <- 0.00000001 #1e-8 247 rows
#padj.cutoff <- 0.000001 #1e-6 548 rows
padj.cutoff <- 0.00001 #1e-5 3969 rows
#padj.cutoff <- 0.001 #1e-3 9211 genes

siglrtRE <- resLRT[resLRT$padj < padj.cutoff & !is.na(resLRT$padj),]

rld_mat        <- assay(rld)
cluster_rlog   <- rld_mat[rownames(siglrtRE),]
meta           <- data.frame(sample_conditions=c("A485_01", "A485_02", "DMSO_01", "DMSO_02"))
rownames(meta) <- colnames(cluster_rlog)
meta$sample_conditions <- factor(meta$sample_conditions, levels=c("DMSO_01", "DMSO_02", "A485_01", "A485_02"))
meta$group <- factor(c("A485", "A485", "DMSO", "DMSO"))
save(cluster_rlog, meta, sample_conditions, file = 'cluster_rlog_pval_1e3.RData')
```
...
write.table(curated_not_different,
            file=paste0('nondynamic_peaks_', Sys.Date(), '.bed'),
...
write.table(bed[,1:3],
            file = paste0('all_peaks_', Sys.Date(), '.bed'),
            sep='\t', col.names=F, row.names=F, quote=F)
```
...
svglite(paste0("degPatterns_1e5_", Sys.Date(), ".svg"),
        bg = "transparent", fix_text_size = FALSE, pointsize = 6)
clusters_all_1e5 <- DEGreport::degPatterns(cluster_rlog, metadata = meta,
                                minc = 100, time = "sample_conditions",
                                col=NULL, eachStep = FALSE)
dev.off()
save(clusters_all_1e5, file = 'clusters_all_minc100_1e5.RData')
```

Plotting
```R
plot_df = clusters_all_1e5$normalized
...
plot_df$sample_conditions[plot_df$sample_conditions == 'A485_01']  <- 2
plot_df$sample_conditions[plot_df$sample_conditions == 'A485_02']  <- 3
...
plot_df$supercluster <- 'up'
plot_df[plot_df$cluster %in% c("cluster2"),'supercluster'] <- 'down'
